...
type RegionOptions = Record<string, unknown>;

interface RegionArgv {
    r?: string | boolean;
    [key: string]: unknown;
}

type MinimstFn = (args: string[]) => RegionArgv;

// modules
const fs = require("fs") as typeof import("fs");
const minimist = require("minimist") as MinimstFn;
const argv = minimist(process.argv.slice(2));

// add to ds9 comment string
const addComment = function(comment: string, key: string | null, val: unknown): string{
    let value = val;
    if( comment !== "" ){
        comment += " ";
    }
...
const parseRegion = function(s: string): void{
    let cmd: string;
    let json: RegionOptions = {};
    let highlite = "";
    let comment = "";
    ...
    if( jarr && jarr[0] ){
        // convert to object
        try{
            json = JSON.parse(jarr[0].trim()) as RegionOptions;
        } catch(_e){
            json = {};
        }
    }
...
// main routine
const convertRegion = function(s: string): void{
    const seprexp = /\n|;/;
...
fs.readFile(argv.r, "ascii", (err: NodeJS.ErrnoException | null, data: string) => {
    if( err ){
