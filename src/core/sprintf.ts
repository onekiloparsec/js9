...
(function(){
    const formatInteger = function(value, base, uppercase){
        let result;
        if( base === 10 ){
    ...
    const pad = function(text, width, flags, prefixLength){
    ...
    const precisionFrom = function(token, nextArg){
    ...
    const widthFrom = function(token, nextArg){
        if( token === undefined || token === null ){
            return undefined;
        }
        if( token === "*" ){
    ...
    const formatSpecifier = function(specifier, value, flags, width, precision){
        const leftAlign = flags.includes("-");
    ...
    const vsprintf = function(format, argv){
        let argIndex = 0;
