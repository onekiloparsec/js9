...
(function(){
    ...
    const installContextMenuStyles = function(){
        ...
        style.textContent = [
            ".context-menu-list{position:absolute;z-index:100000;min-width:220px;margin:0;padding:6px 0;list-style:none;background:#fff;border:1px solid #b9c4d2;border-radius:8px;box-shadow:0 12px 28px rgba(18,34,56,.18);font:13px/1.4 sans-serif;color:#132238;}",
            ".context-menu-item{position:relative;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:7px 14px;white-space:nowrap;cursor:pointer;user-select:none;}",
            ".context-menu-item.context-menu-input{cursor:default;align-items:flex-start;}",
            ".context-menu-item.context-menu-hover{background:#eaf2ff;}",
            ".context-menu-item.context-menu-disabled{opacity:.45;cursor:not-allowed;}",
            ".context-menu-separator{height:1px;margin:6px 10px;background:#d9e0ea;}",
            ".context-menu-item > .context-menu-list{display:none;top:0;left:100%;margin-left:6px;}",
            ".context-menu-item.context-menu-visible > .context-menu-list{display:block;}",
            ".context-menu-item.context-menu-submenu::after{content:'›';color:#6b7a90;}",
            ".context-menu-item label{display:flex;flex-direction:column;gap:4px;width:100%;}",
            ".context-menu-item input[type='text'],.context-menu-item textarea,.context-menu-item select{width:100%;box-sizing:border-box;padding:5px 7px;border:1px solid #b9c4d2;border-radius:4px;background:#fff;color:#132238;}",
            ".context-menu-item input[type='text']:focus,.context-menu-item textarea:focus,.context-menu-item select:focus{outline:2px solid #89aef7;outline-offset:0;}",
            ...
            ".JS9ContextMenuLabel{display:inline-flex;align-items:center;gap:8px;}",
            ".JS9ContextMenuRoot{max-height:min(70vh,680px);overflow:auto;}"
        ].join("");
    ...
    const buildContextMenuTree = function(items, state){
        const menu = document.createElement("ul");
        menu.className = "context-menu-list JS9ContextMenuRoot";
        Object.keys(items || {}).forEach((key) => {
            ...
            if( item && item.disabled ){
                li.classList.add("context-menu-disabled");
            }
            ...
            if( item && item.items ){
                ...
                childMenu = buildContextMenuTree(item.items, state);
                li.appendChild(childMenu);
                li.addEventListener("mouseenter", () => {
                    Array.from(menu.children).forEach((sibling) => {
                        if( sibling !== li ){
                            sibling.classList.remove("context-menu-visible");
                        }
                    });
                    li.classList.add("context-menu-visible");
                });
                li.addEventListener("mouseleave", () => {
                    li.classList.remove("context-menu-visible");
                });
            } else {
            ...
            li.addEventListener("mouseenter", () => {
                if( !li.classList.contains("context-menu-disabled") ){
                    li.classList.add("context-menu-hover");
                }
            });
            li.addEventListener("mouseleave", () => {
                li.classList.remove("context-menu-hover");
            });
            menu.appendChild(li);
        });
        return menu;
    ...
    const openContextMenu = function(config, trigger){
        ...
        menu = buildContextMenuTree(built.items, state);
        menu.dataset.contextMenuId = `js9-context-menu-${++contextMenuId}`;
        opt.menu = wrap(menu);
        opt.menuNode = menu;
        menu.classList.add("context-menu-list");
