...
JS9.Menubar.createMenus = function(){
	...
	let rect;
	const gap = 6;
	triggerNode = opt && opt.trigger ? opt.trigger[0] || opt.trigger : null;
	if( !{}.hasOwnProperty.call(window, "Jupyter") ){
		...
		opt.menu.css("top", rect.bottom + window.pageYOffset + gap);
	    }
	} else {
	    ...
	    if( triggerNode && triggerNode.getBoundingClientRect ){
		rect = triggerNode.getBoundingClientRect();
		opt.menu.css({"left": pos.left+20,
			      "top": rect.bottom + window.pageYOffset + gap});
	    } else {
		opt.menu.css({"left": pos.left+20, "top": pos.top+10+gap});
	    }
	}
    };
    const onhide = () => {
	...
	position: mypos,
		...
		events: {
			...
			if( udisp  ){
			    ...
			    JS9.jupyterFocus(".JS9ContextMenuItem");
			}
		    },
		    hide: (opt) => {
			let obj;
