...
function JS9InstallViewerUtils(JS9) {
    ...
    JS9.raw2FITS = function (raw, opts) {
        ...
        const fixparam = (card, name, val, comm) => {
            ...
            if (name === "XTENSION" && !val) {
                ncard = sprintf("%s  = %20s / %-47s", "SIMPLE", "T", "file does conform to FITS standard");
            }
            else {
    ...
    JS9.hdus2Str = function (hdus) {
        ...
        for (i = 0; i < hdus.length; i++) {
            ...
            t += sprintf("<b>#%d</b>:&#09;<b>name</b>: %s&#09;<b>type</b>: %s", obj.hdu, s, obj.type);
            switch (obj.type) {
                case "image":
    ...
    JS9.parseStaticColors = function (arr) {
        ...
        for (i = 0; i < arr.length; i++) {
            ...
            if (JS9.isArray(a)) {
                ...
                if (!a[0]) {
                    JS9.error(`no color specified: ${arr[i]}`);
                }
    ...
    JS9.lookupStaticColor = (im, val, cache) => {
        ...
        if (im && im.staticObj) {
            ...
            if (val < im.staticObj.colors[0].min) {
                return nocolor;
            }
